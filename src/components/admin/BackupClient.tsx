...
type ExportAction = () => Promise<unknown>
type ImportAction = (data: unknown) => Promise<{ success: boolean; error?: string }>
...
export function BackupClient({ exportAction, importAction }: BackupClientProps) {
    ...
    try {
      ...
      reader.onload = async (event) => {
        try {
          ...
          const result = await importAction(json)
          if (result.success) {
    ...
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>以前エクスポートしたJSONファイルからデータを復元します。</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="relative">
              ...
              />
              <Button variant="outline" disabled={isImporting} className="w-full gap-2">
                {isImporting ? 'インポート中...' : (
                  <>
      ...
      <Card className="border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20 dark:border-yellow-900">
        ...
        <CardContent className="text-sm text-yellow-700 dark:text-yellow-300 space-y-2">
          <p>インポートを行うと、同じIDを持つ既存のデータは上書きされます。安全のため、インポート前に現在のデータをエクスポートしておくことをお勧めします。</p>
          <p>現在、画像ファイル自体はバックアップに含まれません（DB内のURL参照のみが保存されます）。</p>
        </CardContent>
      </Card>
